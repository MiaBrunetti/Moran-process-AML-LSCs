function varargout=sigstar(groups,stats,nosort)
    ...
    for ii = 1:length(groups)
        ...
        elseif iscell(grp) %Handle string pairs or string/index pairs
            ...
            if ischar(grp{1})
                idx_a = matches(xtl,grp{1});
                a = find(idx_a == 1);
            elseif isnumeric(grp{1})
                a=grp{1};
            end
            if ischar(grp{2})
                idx_b = matches(xtl,grp{2});
                b = find(idx_b == 1);
    ...
    %for all bars.
    yd=(max(ylim)-min(ylim))*0.01; %Ticks are 1% of the y axis range
    for ii=1:length(groups)
        y=get(H(ii,1),'YData');
        y(1)=y(1)-yd;
        y(4)=y(4)-yd;   
...
function H=makeSignificanceBar(x,y,p)
    ...
    starY=mean(y)+(max(ylim)-min(ylim))*offset;
    ...
    Y=ylim;
    if Y(2)<starY
        ylim([Y(1),starY+(max(Y)-min(Y))*0.05])
    end
